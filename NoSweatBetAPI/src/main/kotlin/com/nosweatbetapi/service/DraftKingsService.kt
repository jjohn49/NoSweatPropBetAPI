...
class DraftKingsService : SportsBookService{
    ...
    private fun scrapeBets(type: TeamBetType,teams: MutableList<WebElement>): MutableList<TeamBet> {
        ...
        for (i in teams.indices step 2) {
            ...
            val pointSpread: Float = if (pointSpreadText != "pk") pointSpreadText.removeRange(0, 1).toFloat() else 0f
    ...
    override fun getCurrentNBASpreads(): SportsBookTeamBets{
        return this.getCurrentBets("https://sportsbook.draftkings.com/leagues/basketball/nba", TeamBetType.Spread)
    }
    override fun getCurrentNBAOverUnders(): SportsBookTeamBets {
        return this.getCurrentBets("https://sportsbook.draftkings.com/leagues/basketball/nba", TeamBetType.OverUnder)
    }
